...
class SwitchStats():
    ...
    def __format_bytes(self, number):
        ...
        if number < 1000 and number > -1000:
            ...
            return (sign,int(float(number)/1000000),int(float(number)/10000)-(int(float(number)/1000000)*100),'MB')
        elif number < 1000000000000 and number > -1000000000000:
            return (sign,int(float(number)/1000000000),int(float(number)/10000000)-(int(float(number)/1000000000)*100),'GB')
        else:
            return (sign,int(float(number)/1000000000000),int(float(number)/10000000000)-(int(float(number)/1000000000000)*100),'TB')
    ...
    def updateSDNStats(self):
        ...
        for portNumber in self.ports:
            ...
            t1_sdn = p['sdn_rx_bytes_buffer_index']
            p['sdn_rx_bytes_buffer_index'] = (p['sdn_rx_bytes_buffer_index']+1)%DELTA_WINDOW
            p['sdn_rx_bytes_buffer'][t1_sdn] = self.__getSDNRxBytes(portNumber) + (self.__seconds_from_start * LLDP_NOISE_BYTE_S * rx_noise)
            t1_sdn = p['sdn_tx_bytes_buffer_index']
            p['sdn_tx_bytes_buffer_index'] = (p['sdn_tx_bytes_buffer_index']+1)%DELTA_WINDOW
            p['sdn_tx_bytes_buffer'][t1_sdn] = self.__getSDNTxBytes(portNumber) - (self.__seconds_from_start * LLDP_NOISE_BYTE_S)
            t1_sdn = p['sdn_rx_packets_buffer_index']
            p['sdn_rx_packets_buffer_index'] = (p['sdn_rx_packets_buffer_index']+1)%DELTA_WINDOW
            p['sdn_rx_packets_buffer'][t1_sdn] = self.__getSDNRxPackets(portNumber) + (self.__seconds_from_start * LLDP_NOISE_PACK_S * rx_noise)
            t1_sdn = p['sdn_tx_packets_buffer_index']
            p['sdn_tx_packets_buffer_index'] = (p['sdn_tx_packets_buffer_index']+1)%DELTA_WINDOW
            p['sdn_tx_packets_buffer'][t1_sdn] = self.__getSDNTxPackets(portNumber) - (self.__seconds_from_start * LLDP_NOISE_PACK_S)